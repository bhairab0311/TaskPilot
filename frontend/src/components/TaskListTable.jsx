...
const TaskListTable = ({ tableData }) => {
  ...
  const getStatusBadgeColor = (status) => {
    switch (status) {
      ...
      case "In Progress":
        return "bg-cycn-100 text-cycn-500 border border-cycn-200"; 
      default:
  ...
  const getPriorityBadgeColor = (priority) => {
    switch (priority) {
      ...
      case "Low":
        return "bg-green-100  text-green-500 border border-green-200"; // Fixed missing classes
      default:
        return "bg-gray-100 text-gray-500 border border-gray-200"; // Ensures default fallback
    }
  };
  return (
    ...
    <table className="min-w-full">
      <thead>
        <tr className="text-left">
          <th className="py-3 px-4 text-gray-600 font-medium text-[13px]">Name</th>
